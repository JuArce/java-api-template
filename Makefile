...
create_db:
	...
	@if [ "$(database)" = "java-api-template" ]; then echo "Using default container name: 'java-api-template'."; fi

	docker run --name $(container) -e POSTGRES_PASSWORD=$(password) -e POSTGRES_USER=$(user) -p 5432:5432 -d postgres
	sleep 1  # Wait for the PostgreSQL container to start (you can adjust this as needed)
